...
✞ Enter your whatsapp number with the country code e.g;  254745789590 then tap submit 
✞Alpha Md owner , *kelvin* ,will send you the code immediately.Copy the code and whatsapp will bring a notification 
✞Tap on the notification and paste the code that Alpha md owner sent you.
✞ After a successful login Alpha Md owner 😂 Kelvin will send you a *session id* .some will wonder where the hell is that session ,,,...It is just right at your inbox /dm {your own number for whatsappp} 😂
...
let menuMsg = `
   ...
   if (lien.match(/\.(mp4|gif)$/i)) {
    try {
        zk.sendMessage(dest, { video: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *kelvin*, déveloper Kelvin Tech" , gifPlayback : true }, { quoted: ms });
    }
    catch (e) {
...
// Vérification pour .jpeg ou .png
    ...
    try {
        zk.sendMessage(dest, { image: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *kelvin*, déveloper Kelvin Tech" }, { quoted: ms });
    }
    catch (e) {
        console.log("🥵🥵 Menu erreur " + e);
        repondre("🥵🥵 Menu erreur " + e);
